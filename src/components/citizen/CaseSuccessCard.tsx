...
export const CaseSuccessCard: React.FC<CaseSuccessCardProps> = ({ caseData }) => {
  ...
  return (
    <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-white/10 max-w-2xl mx-auto space-y-8 shadow-2xl text-center animate-fade-in">
      
      {/* Top Banner Icon */}
      <div className="space-y-3">
        <div className="w-16 h-16 rounded-full bg-emerald-500/20 text-emerald-400 border border-emerald-500/40 mx-auto flex items-center justify-center shadow-glow-purple">
          <CheckCircle2 className="w-8 h-8" />
        </div>
        <h2 className="text-2xl font-extrabold text-white">Your Report Has Been Submitted Safely</h2>
      ...
      <div className="p-4 rounded-2xl bg-slate-900/90 border border-slate-800 space-y-2">
        <span className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Unique Case Reference ID</span>
      ...
      <div className="p-5 rounded-2xl bg-slate-950/60 border border-slate-800 text-left space-y-3">
        <span className="text-xs font-bold text-slate-300 uppercase tracking-wider block">Initiated Case Milestones</span>
        <div className="space-y-2 text-xs">
          ...
          <div className="flex items-center space-x-2.5 text-purple-300 font-medium">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
        ...
        <Link
          to="/responder"
          className="py-3.5 px-5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-semibold text-sm border border-slate-700 flex items-center justify-center space-x-2 transition-all"
        >
