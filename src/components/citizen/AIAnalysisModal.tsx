...
export const AIAnalysisModal: React.FC<AIAnalysisModalProps> = ({ analysis, onProceed }) => {
  ...
  return (
    <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-white/10 max-w-2xl mx-auto space-y-6 shadow-2xl animate-fade-in">
      ...
      <div className="flex items-center space-x-3 border-b border-slate-800 pb-4">
        ...
        <div>
          <div className="flex items-center space-x-2">
            <h3 className="text-lg font-bold text-white">AI-Assisted Safety Triage</h3>
            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-brand-purple/20 text-purple-300 border border-brand-purple/30 flex items-center gap-1">
              <Sparkles className="w-3 h-3" />
              Rakshak Core AI
            </span>
          </div>
          <p className="text-xs text-slate-400">Real-time risk classification & responder dispatch recommendation</p>
        </div>
      </div>
      ...
      {!isComplete ? (
        <div className="py-8 space-y-4 text-center">
          <div className="w-14 h-14 mx-auto rounded-2xl bg-brand-purple/20 border border-brand-purple/40 flex items-center justify-center text-brand-purple animate-bounce">
          ...
          <div className="space-y-2">
            ...
            <div className="w-full bg-slate-900 rounded-full h-2 overflow-hidden border border-slate-800 max-w-md mx-auto">
              <div
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="p-6 rounded-2xl bg-slate-900/90 border border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="space-y-1">
              <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Assessed Risk Score</span>
              <div className="flex items-baseline space-x-2">
                <span className="text-4xl font-extrabold text-white font-mono">{analysis.riskScore}</span>
          ...
          <div className="p-3.5 rounded-xl bg-purple-500/10 border border-purple-500/30 text-purple-200 text-xs flex items-start space-x-2.5">
            <AlertCircle className="w-4 h-4 text-purple-400 flex-shrink-0 mt-0.5" />
            <div>
              <span className="font-bold text-purple-300 block">Advisory AI Assessment</span>
          ...
          <div className="space-y-2">
            <h4 className="text-xs font-bold text-slate-300 uppercase tracking-wider">Detected Risk Indicators</h4>
            <div className="space-y-1.5">
              {analysis.indicators.map((ind, idx) => (
                <div key={idx} className="p-3 rounded-xl bg-slate-900/60 border border-slate-800 text-xs text-slate-200 flex items-center space-x-2">
          ...
          <div className="border border-slate-800 rounded-2xl overflow-hidden bg-slate-900/50">
            <button
              ...
              className="w-full p-4 text-left flex items-center justify-between text-xs font-bold text-slate-200 hover:bg-slate-800/50 transition-colors"
            >
              <div className="flex items-center space-x-2">
            ...
            {expandedWhy && (
              <div className="p-4 pt-0 border-t border-slate-800/60 space-y-2 text-xs text-slate-300">
