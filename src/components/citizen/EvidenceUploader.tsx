...
export const EvidenceUploader: React.FC<EvidenceUploaderProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-200 flex items-start space-x-3 text-xs">
        <ShieldAlert className="w-5 h-5 text-amber-400 flex-shrink-0 mt-0.5" />
        <div className="space-y-1">
          <span className="font-bold text-amber-300 block">Safety Disclaimer</span>
          <p>{t('photoWarning')}</p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        {!photoUrl ? (
          <div className="border-2 border-dashed border-slate-800 rounded-2xl p-6 text-center space-y-3 bg-slate-900/50 hover:border-brand-purple/50 transition-all">
            <div className="w-12 h-12 rounded-full bg-brand-purple/20 text-brand-purple mx-auto flex items-center justify-center">
              <Camera className="w-6 h-6" />
            </div>
            <div>
              <p className="text-xs text-slate-300 font-medium">Click to upload photo or choose demo image</p>
              <p className="text-[11px] text-slate-500 mt-0.5">PNG, JPG up to 10MB</p>
            </div>
            <button
              type="button"
              onClick={handleSimulateUpload}
              className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold rounded-xl border border-slate-700 transition-all inline-flex items-center space-x-2"
            >
          ...
          <div className="relative rounded-2xl overflow-hidden border border-slate-800 bg-slate-900 p-3 flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <div className="relative w-16 h-16 rounded-xl overflow-hidden bg-slate-950 flex-shrink-0">
                <img
                  src={photoUrl}
              ...
              <div className="space-y-1">
                <div className="text-xs font-semibold text-white">Synthetic Evidence Attached</div>
                <button
                  type="button"
                  onClick={() => onChangeBlurred(!isBlurred)}
                  className="text-[11px] text-brand-purple hover:text-purple-300 flex items-center space-x-1 font-medium transition-colors"
                >
      ...
      <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <span className="text-xs font-bold text-white block">{t('anonymousToggle')}</span>
            <span className="text-[11px] text-slate-400">No name, phone number, or account required.</span>
          </div>
        </div>
        ...
        <button
          ...
          className={`w-12 h-6 rounded-full transition-colors p-1 relative ${
            isAnonymous ? 'bg-brand-purple' : 'bg-slate-800'
          }`}
        >
          <div
