...
import { MapPin, Navigation, Eye, Flame, Layers, Shield, Filter, Crosshair } from 'lucide-react';
...
export const MapView: React.FC<MapViewProps> = ({
  ...
  const [hoveredCase, setHoveredCase] = useState<CaseModel | null>(null);
  ...
  // Map station markers relative to schematic transit grid layout
  const getMarkerPosition = (index: number, total: number, coords?: { lat: number; lng: number }) => {
    if (coords) {
      // Map lat/lng range roughly into 10% to 90% space
  ...
  return (
    <div className="relative w-full h-[540px] rounded-2xl overflow-hidden bg-slate-950 border border-white/10 shadow-2xl flex flex-col">
      {/* Top Map Toolbar */}
      <div className="absolute top-4 left-4 right-4 z-20 flex flex-wrap items-center justify-between gap-2 pointer-events-none">
        
        {/* Title Badge */}
        <div className="pointer-events-auto bg-slate-900/90 backdrop-blur-md px-3.5 py-1.5 rounded-xl border border-slate-800 text-xs font-semibold text-slate-200 flex items-center space-x-2 shadow-lg">
          <Navigation className="w-4 h-4 text-brand-purple animate-pulse" />
          <span>Transit Hub Incident Radar</span>
          <span className="bg-brand-purple/20 text-purple-300 font-mono text-[10px] px-2 py-0.5 rounded-full border border-brand-purple/30">
            {filteredCases.length} Live Clusters
          </span>
        </div>

        {/* Controls: Heatmap Toggle & Risk Filter */}
        <div className="pointer-events-auto flex items-center space-x-2">
          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-xl px-2 py-1 text-xs text-slate-300 flex items-center space-x-1">
      ...
      {/* Map Canvas Visual (Synthetic Styled Grid Map) */}
      <div className="relative w-full h-full bg-[#0B0F19] overflow-hidden select-none">
        
        {/* Synthetic Map Background Grid & Vector Routes */}
        ...
        {filteredCases.map((c, idx) => {
          ...
          return (
            ...
            >
              ...
              {/* Marker Icon */}
              <div
                className={`relative p-2.5 rounded-2xl flex items-center justify-center transition-all duration-300 transform group-hover:scale-125 border-2 border-white/20 ${markerBg} ${
              ...
              {/* Station Label */}
              <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 whitespace-nowrap bg-slate-900/90 text-[10px] font-bold px-2 py-0.5 rounded-full border border-slate-800 text-slate-200 pointer-events-none shadow-md">
        ...
        {/* Bottom Left Detail Drawer / Active Case Card */}
        {activeCase && (
          <div className="absolute bottom-4 left-4 z-20 max-w-sm w-full glass-panel p-4 rounded-2xl border border-white/10 shadow-2xl animate-fade-in space-y-3">
            <div className="flex items-center justify-between border-b border-slate-800 pb-2">
              <div className="flex items-center space-x-2">
                <span className="font-mono text-xs font-bold text-slate-300">{activeCase.id}</span>
            ...
            <div>
              <h4 className="text-sm font-bold text-white line-clamp-1">{activeCase.report.location}</h4>
              <p className="text-xs text-slate-300 line-clamp-2 mt-1">{activeCase.report.description}</p>
            </div>
