  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-2">
        ...
        <span className="text-[10px] text-slate-400 dark:text-slate-500 font-mono">Live Audit Record</span>
      </div>
          ...
          <div key={event.id || idx} className="relative flex items-start space-x-3 group">
            ...
            <div className="space-y-0.5 flex-1">
              <div className="flex items-center justify-between">
                <span className={`text-xs font-bold ${event.completed ? 'text-slate-900 dark:text-white' : 'text-slate-400 dark:text-slate-500'}`}>
                  {event.title}
                </span>
