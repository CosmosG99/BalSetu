  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10 space-y-8">
      ...
      <div className="text-center space-y-3">
        <div className="w-12 h-12 rounded-2xl bg-brand-purple/20 text-brand-purple border border-brand-purple/40 mx-auto flex items-center justify-center">
          <Search className="w-6 h-6" />
        </div>
        <h1 className="text-3xl font-extrabold text-white">{t('trackTitle')}</h1>
        <p className="text-xs text-slate-400 max-w-md mx-auto">{t('trackSubtitle')}</p>
        ...
        <form onSubmit={handleSearch} className="flex items-center space-x-2 max-w-md mx-auto pt-2">
          <div className="relative flex-1">
            <input
              ...
              className="w-full pl-4 pr-4 py-3 bg-slate-900/90 border border-slate-800 rounded-xl text-sm font-mono text-white placeholder-slate-500 focus:outline-none focus:border-brand-purple"
            />
          </div>
          <button
            type="submit"
      ...
      {activeCase ? (
        <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-white/10 space-y-6 shadow-2xl animate-fade-in">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-slate-800 pb-4 gap-3">
            <div>
              <span className="text-[10px] text-slate-400 font-mono uppercase">Case Reference</span>
              <h2 className="text-2xl font-extrabold text-white font-mono">{activeCase.id}</h2>
              ...
              {activeCase.timeline.map((step) => (
                <div key={step.id} className="relative space-y-0.5">
                  ...
                  <div className="flex items-center justify-between text-xs">
                    <span className={`font-bold ${step.completed ? 'text-white' : 'text-slate-500'}`}>
                      {step.title}
                    </span>
        ...
        <div className="glass-panel p-8 rounded-3xl border border-white/10 text-center space-y-3">
          ...
          <h3 className="text-lg font-bold text-white">No Case Found for "{paramCaseId || inputCaseId}"</h3>
          <p className="text-xs text-slate-400">
            Please check the Case ID reference string and try again. Example: <span className="font-mono text-purple-300">RB-2026-10482</span>
          </p>
        </div>
      )}
