  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex items-center justify-between">
        <Link
          to="/responder"
          className="inline-flex items-center space-x-1.5 text-slate-400 hover:text-white text-xs font-semibold transition-colors"
        >
          <ArrowLeft className="w-4 h-4" />
      ...
      <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-white/10 space-y-4 shadow-2xl">
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 border-b border-slate-800 pb-4">
          <div>
            <div className="flex items-center space-x-3">
              <h1 className="text-3xl font-extrabold text-white font-mono">{caseData.id}</h1>
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs text-slate-300 pt-2">
              <span className="flex items-center space-x-1">
                ...
                <span className="font-bold text-white">{caseData.report.location}</span>
              </span>
              <span className="flex items-center space-x-1">
              ...
              <span className="text-slate-400">
                Reporter: <span className="text-purple-300 font-semibold">{caseData.report.isAnonymous ? 'Anonymous Citizen' : caseData.report.reporterRole}</span>
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="py-2.5 px-4 rounded-xl bg-red-500/20 hover:bg-red-500/30 text-red-300 font-bold text-xs border border-red-500/40 transition-colors"
            >
              Escalate Case
            </button>
          </div>
        ...
        {caseData.assignedResponder && (
          <div className="p-3 rounded-xl bg-purple-500/10 border border-purple-500/30 text-xs text-purple-200 flex items-center space-x-2 font-medium">
            <UserCheck className="w-4 h-4 text-brand-purple" />
            <span>Assigned Ground Responder: <strong className="text-white">{caseData.assignedResponder}</strong></span>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="glass-panel p-6 rounded-2xl border border-white/10 space-y-4 shadow-xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="text-sm font-bold text-white uppercase tracking-wider">AI-Assisted Triage Assessment</h3>
              </div>
              <span className="text-[10px] font-bold text-purple-300 bg-brand-purple/20 px-2.5 py-0.5 rounded-full border border-brand-purple/30">
            ...
            <div className="p-3 rounded-xl bg-slate-900/90 border border-slate-800 text-xs text-slate-300 space-y-1">
              <span className="font-semibold text-amber-400 block">Advisory Disclaimer</span>
              <p>"AI output is advisory. Mandatory human verification is required."</p>
            </div>

            <div className="space-y-2 text-xs">
              <span className="font-bold text-slate-300 uppercase tracking-wider block">Detected Indicators</span>
              <div className="space-y-1.5">
                {caseData.aiAnalysis.indicators.map((ind, idx) => (
                  <div key={idx} className="p-2.5 rounded-xl bg-slate-900/60 border border-slate-800 text-slate-200 flex items-center space-x-2">
            ...
            <div className="space-y-1 text-xs pt-1">
              <span className="font-bold text-slate-300 uppercase tracking-wider block">Why Flagged?</span>
          ...
          <div className="glass-panel p-6 rounded-2xl border border-white/10 space-y-3 shadow-xl">
            <h3 className="text-xs font-bold text-white uppercase tracking-wider">Submitted Incident Description</h3>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs pt-1 font-mono">
              <div className="p-2.5 rounded-xl bg-slate-900 border border-slate-800">
                ...
                <span className="text-white font-bold">{caseData.report.approxAge || 'Unspecified'}</span>
              </div>
              <div className="p-2.5 rounded-xl bg-slate-900 border border-slate-800">
                <span className="text-slate-500 block text-[10px]">Gender</span>
                <span className="text-white font-bold">{caseData.report.apparentGender || 'Unspecified'}</span>
              </div>
              <div className="p-2.5 rounded-xl bg-slate-900 border border-slate-800">
                <span className="text-slate-500 block text-[10px]">Clothing</span>
                <span className="text-white font-bold truncate block">{caseData.report.clothing || 'Unspecified'}</span>
              </div>
              <div className="p-2.5 rounded-xl bg-slate-900 border border-slate-800">
                <span className="text-slate-500 block text-[10px]">Platform / Gate</span>
                <span className="text-white font-bold">{caseData.report.platformOrGate || 'Unspecified'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="glass-panel p-6 rounded-2xl border border-white/10 space-y-4 shadow-xl">
            ...
            <form onSubmit={handleAddNote} className="space-y-3">
              <textarea
                ...
                className="w-full p-3.5 bg-slate-900 border border-slate-800 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-brand-purple"
              />
              <div className="flex items-center justify-between">
                <input
                  type="text"
      ...
      {statusModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="glass-panel p-6 rounded-3xl border border-white/10 max-w-md w-full space-y-4 shadow-2xl animate-fade-in">
            <h3 className="text-lg font-bold text-white">Update Workflow Status</h3>
            <p className="text-xs text-slate-400">Select the new operational state for Case {caseData.id}:</p>
