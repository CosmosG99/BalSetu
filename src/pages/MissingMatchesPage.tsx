...
export const MissingMatchesPage: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <h1 className="text-2xl font-extrabold text-white">Potential Match Assistance</h1>
          </div>
      ...
      <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-200 flex items-start space-x-3 text-xs shadow-lg">
        <AlertTriangle className="w-5 h-5 text-amber-400 flex-shrink-0 mt-0.5" />
        <div>
          <strong className="text-amber-300 block text-sm font-bold">MANDATORY ADVISORY WARNING</strong>
          <p>
      ...
      <div className="glass-panel p-6 rounded-3xl border border-white/10 space-y-4 shadow-xl">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-slate-400 font-mono">1 face detected</span>
        </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {results.map((item, idx) => (
            <div
              key={item.id}
              className="glass-panel glass-panel-hover p-5 rounded-2xl border border-white/10 space-y-3 shadow-lg flex flex-col justify-between"
            >
              <div className="space-y-3">
                {/* Photo & Similarity Pill */}
                <div className="relative rounded-xl overflow-hidden bg-slate-950 h-44">
                  <img src={item.photoUrl} alt={item.syntheticName} className="w-full h-full object-cover" />
                  <div className="absolute top-2 right-2 bg-slate-900/90 text-emerald-400 font-mono font-bold text-xs px-2.5 py-1 rounded-full border border-emerald-500/40">
                ...
                <div>
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[11px] text-slate-400 font-mono">{item.gender}, Age {item.age}</span>
                  </div>
                  <h4 className="text-sm font-bold text-white line-clamp-1">{item.syntheticName}</h4>
                  <p className="text-xs text-slate-400 mt-1">Last Seen: <span className="text-slate-200 font-medium">{item.lastKnownLocation}</span></p>
                  <p className="text-[11px] text-slate-500 line-clamp-2 mt-1">{item.description}</p>
                </div>
              </div>

              <button className="w-full py-2 px-3 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs border border-slate-700 transition-colors flex items-center justify-center space-x-1.5">
